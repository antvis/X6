...
export class CellEditor extends Disposable {
      ...
      if (!this.autoSize || state.style.overflow === 'fill') {
        ...
        if (isEdge) {
          this.bounds = new Rectangle(
            state.absoluteOffset.x, state.absoluteOffset.y, 0, 0,
          )
          if (lw != null) {
          ...
          bds = (state.shape != null && hpos === 'center' && vpos === 'middle')
          ...
          if (
            ...
            bds = new Rectangle(
              bds.x + spacingLeft,
              bds.y + spacingTop,
              bds.width - ((hpos === 'center' && lw == null) ? (spacingLeft + spacingRight) : 0),
  ...
  getEditorBounds(state: State) {
    ...
    if (
      ...
      result = (state.shape != null && hpos === 'center' && vpos === 'middle')
