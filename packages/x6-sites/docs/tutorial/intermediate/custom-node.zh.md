...
回到代码层面，自定义节点实际上是从基础节点派生（继承）出我们自己的节点，并覆盖基础节点的某些选项和方法。

拿内置的 Rect 节点来举例，自定义节点可以分以下三步走。
...
第二步，配置节点。调用继承的静态方法 `config(options)` 来配置[节点选项](../node/#选项)的默认值、[自定义选项](../basic/cell#自定义选项)和[自定义属性]()，最常用选项的是通过 [markup](../basic/cell#markup) 来指定节点默认的 SVG/HTML 结构，通过 [attrs](../basic/cell#attrs-1) 来指定节点的默认属性样式。
...
第三步，注册节点。调用 Graph 的静态方法 registerNode 来注册节点，注册以后就可以像使用内置节点那样来使用节点。
...
该方法将其调用者（如上面的 `Rect`）作为基类，继承出一个新的节点，然后调用新节点的静态方法 `config` 来配置默认选项。需要注意的是，上面代码生成的 `RedRect` 类的类名并不是 `'RedRect'`，而是系统自动生成的类名，我们可以通过 `constructorName` 选项来指定新节点的类名。

```ts
const RedRect = Rect.define({
  constructorName: 'RedRect',
  attrs: {
    body: {
      stroke: 'red',
    },
...
如果我们同时提供了 `shape` 选项，那么系统将自动为你注册节点。

```ts
Rect.define({
  shape: 'redRect', // 自动注册名为 'redRect' 的节点，并且节点类名为 'RedRect'。
  attrs: {
    body: {
      stroke: 'red',
    },
...
当省略 `constructorName` 时 `shape` 的大驼峰形式（CamelCase）将作新节点的类名，也就是说上面代码定义的节点类名为 `'RedRect'`。

除了 `constructorName` 和 `shape` 两个特殊选项外，其他选项都与 [config 方法](#理论基础)的选项保持一致。下表是 `define` 方法支持的选项。

| 名称            | 是否必选 | 类型   | 说明                                                                             |
|-----------------|----------|--------|--------------------------------------------------------------------------------|
| constructorName | 否       | String | 类名。                                                                            |
| shape           | 否       | String | 自动注册的节点名，当 `constructorName` 缺省时其大驼峰（CamelCase）形式也将作为类名。 |
| ...others       |          |        | [config 方法](#理论基础)的选项。                                                  |
