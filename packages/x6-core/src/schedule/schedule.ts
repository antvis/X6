...
export class Schedule {
  ...
  protected processEdgeOnTerminalVisibleChanged(node: Cell, visible: boolean) {
    ...
    this.model.getConnectedEdges(node).forEach((edge: any) => {
      // todo
  ...
  scheduleViewUpdate(
    ...
    this.onViewUpdated(view as CellView, flag, options)
  }
    ...
    main: for (let p = 0, n = priorities.length; p < n; p += 1) {
      ...
      for (const cid in cache) {
        ...
        if (leftoverFlag > 0) {
          ...
          if (
            !this.onViewPostponed(cellView, leftoverFlag, options) ||
            cache[cid]
          ) {
            postponedCount += 1
            empty = false
  ...
  sortViewsExact() {
    ...
    const childs = this.view.stage.children
    const elems = []
    for (let i = 0, len = childs.length; i < len; i += 1) {
      if (childs[i].hasAttribute('data-cell-id')) {
        elems.push(childs[i])
      }
    }
    const model = this.model
  ...
  findViewsFromPoint(p: Point.PointLike) {
    ...
    return this.model
      .getCells()
      .map((cell: any) => this.findViewByCell(cell)) // todo
      .filter((view: any) => {
        // todo
        if (view != null) {
  ...
  findEdgeViewsInArea(
    ...
    return this.model
      .getEdges()
      .map((edge: any) => this.findViewByCell(edge)) // todo
      .filter((view: any) => {
        // todo
        if (view) {
  ...
  findViewsInArea(
    ...
    return this.model
      .getNodes()
      .map((node: any) => this.findViewByCell(node)) // todo
      .filter((view: any) => {
        // todo
        if (view) {
          const bbox = Util.getBBox(view.container as SVGElement, {
...
export namespace Schedule {
  ...
  export interface UpdateViewsAsyncOptions extends UpdateViewOptions {
    before?: (this: any, graph: any) => void // todo
    after?: (this: any, graph: any) => void // todo
    /**
