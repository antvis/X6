...
import { Cell, util, Geometry, FontStyle, VAlign, Align } from '../../../../../src'
...
export class FormatCell
  ...
  onLeftChanged = (x: number) => {
    if (x != null) {
      this.setState({ x })
      this.executeCommand('updateGeometry', new Geometry(
        x,
        this.state.y,
        this.state.w,
        this.state.h,
        ...
        <div className="x6-editor-format-section ">
          <div className="section-item">
            <Button
              ...
              onClick={() => { this.executeCommand('toFront') }}
            >
              To Front
            </Button>
            <Button
              ...
              onClick={() => { this.executeCommand('toBack') }}
            >
              To Back
            </Button>
          </div>
        ...
        <div className="x6-editor-format-section">
          ...
          <div className="section-item">
            ...
            >
              {
                [
                  'Helvetica', 'Verdana', 'Times New Roman', 'Garamond',
                  'Comic Sans MS', 'Courier New', 'Georgia', 'Lucida Console',
                  'Tahoma'
                ].map(name => (
          ...
          <div className="section-item">
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': FontStyle.isBold(this.state.fontStyle) })}
              icon="bold"
              onClick={this.toggleBold}
            />
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': FontStyle.isItalic(this.state.fontStyle) })}
              icon="italic"
              onClick={this.toggleItalic}
            />
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': FontStyle.isUnderlined(this.state.fontStyle) })}
              icon="underline"
          ...
          <div className="section-item">
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': this.state.align === 'left' })}
              icon="align-left"
              onClick={() => { this.updateAlign('left') }}
            />
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': this.state.align === 'center' })}
              icon="align-center"
              onClick={() => { this.updateAlign('center') }}
            />
            <Button
              className={classnames('ant-btn-flag', { 'ant-btn-checked': this.state.align === 'right' })}
              icon="align-right"
              style={{ marginRight: 23 }}
              onClick={() => { this.updateAlign('right') }}
            />
            <Button
              className={classnames('ant-btn-flag', 'ant-btn-icon-only', { 'ant-btn-checked': this.state.valign === 'top' })}
              onClick={() => { this.updateVAlign('top') }}
            >
              <i className="anticon">
                <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
                  <path d="M169.984 128h683.99v86.016h-683.99V128zm172.032 342.016L512 298.026l169.984 171.99h-128V896h-84.01V470.016h-128z" />
                </svg>
              </i>
            </Button>
            <Button
              className={classnames('ant-btn-flag', 'ant-btn-icon-only', { 'ant-btn-checked': this.state.valign === 'middle' })}
              onClick={() => { this.updateVAlign('middle') }}
            >
              <i className="anticon">
                <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
            ...
            <Button
              className={classnames('ant-btn-flag', 'ant-btn-icon-only', { 'ant-btn-checked': this.state.valign === 'bottom' })}
              style={{ marginRight: 0 }}
              onClick={() => { this.updateVAlign('bottom') }}
            >
              <i className="anticon">
                <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
            ...
            >
              {
                [
                  ...
                  <Select.Option key={p} value={p.toLowerCase()}>{p}</Select.Option>
                ))
              }
            </Select>
          </div>
