  ...
  describe('attr', () => {
    describe('#styleToObject', () => {
      it('should parse style string to object', () => {
        ...
        expect(Dom.styleToObject('fill=red; stroke=blue')).toEqual(ret)
        expect(Dom.styleToObject('fill=red; stroke=blue;')).toEqual(ret)
      })

      it('should parse key-only string', () => {
        expect(Dom.styleToObject(';fill=red;;stroke')).toEqual({
          fill: 'red',
          stroke: '',
        })
      })
      ...
      it('should ingore empty section', () => {
        expect(Dom.styleToObject(';fill=red;;')).toEqual({ fill: 'red' })
      })

      it('should parse empty string to empty object', () => {
        expect(Dom.styleToObject('')).toEqual({})
      })
    })

    describe('#mergeAttrs', () => {
      it('shoule merge attrs by extend', () => {
        expect(Dom.mergeAttrs({ x: 5, y: 10 }, { y: 20 })).toEqual({
          x: 5,
          y: 20,
        })
      })
