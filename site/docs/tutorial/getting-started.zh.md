...
在项目中安装或者引入完成 X6 之后就可以开始使用了，我们将以一个简单的例子开始体验 X6，再开始之前也强烈推荐先学习一些 [SVG 基础知识](https://codepen.io/HunorMarton/full/PoGbgqj)，不过没有SVG相关知识也可以轻松上手～
...
有了画布之后就可以往画布中添加我们需要的 **节点** 和 **边** 了，X6 支持 JSON 格式数据，该对象中 `nodes` 代表节点数据，`edges` 代表边数据，可以使用 `attrs` 属性来定制节点和边的样式（可以类比 CSS）。

这样我们就得到了一个基本的带点和边的X6画布：
```js | ob { inject: true }
import { Graph } from '@antv/x6'
...
在上面的例子中，都使用了内置的`rect`节点，除此之外 X6 还支持使用框架组件来自定义节点，例如使用 `React` 组件 `Vue` 组件来当作画布中我们看到的一个个节点，这些进阶内容我们会在后续的章节中详细介绍，你也可以提前了解：
