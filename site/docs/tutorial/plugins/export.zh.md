...
| copyStyles         | boolean                                    | `true` |      | 是否复制外部样式表中的样式，默认是 true。开启后会将节点的计算样式按差异内联到导出的 SVG，保证导出效果与页面视觉一致，但有一定性能开销，若追求速度或样式已通过 attrs 指定，可设为 false；也可配合 `stylesheet` 注入必要 CSS。 |
