...
import { v1, v, Point, Angle } from '@antv/x6'
import '../../index.less'
import '../index.less'

class ConveyorNode extends v1.Node {
  addPallet() {
...
const innerRect = v('rect').addClass('rect-inner').node
const outerRect = v('rect').addClass('rect-outer').node

class ConveyorNodeView extends v1.NodeView<ConveyorNode> {
  ...
  updatePallet() {
    ...
    v.attr(this.svgInnerRect, 'fill', palletColor)
  }

  updateRectsDimensions() {
    var node = this.cell
    v.attr(this.svgOuterRect, node.getOuterRectBBox())
    v.attr(this.svgInnerRect, node.getInnerRectBBox())
  }
  ...
  translate() {
    ...
    v.translate(this.container, position.x, position.y, { absolute: true })
  }
  ...
  rotate() {
    ...
    v.rotate(this.container, angle, size.width / 2, size.height / 2, {
      absolute: true,
    })
  }
}
...
v1.NodeRegistry.register('performance_conveyor', ConveyorNode)
v1.ViewRegistry.register('performance_conveyor_view', ConveyorNodeView)
