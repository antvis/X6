...
import styles from './experiment-graph.less';
...
class ExperimentGraph extends GraphCore<BaseNode, BaseEdge> {
  ...
  > = new BehaviorSubject<NExperimentGraph.ModalParams | undefined>(
    null as any,
  );
  ...
  async onConnectNode(args: any) {
    ...
    const { source, target, id } = edge as any;
    if (isNew) {
      // 处理边虚线样式更新的问题。
  ...
  requestDeleteNodes = async (ids: string[] | string) => {
    ...
    if (this.graph && nodeInstanceIds.length) {
      ...
      this.delExperimentGraphShape(nodeInstanceIds, []);
      return { success: true };
    }
    return { success: false };
  };
  ...
  requestDeleteEdges = async (edges: BaseEdge | BaseEdge[]) => {
    ...
    this.delExperimentGraphShape(targetEdges.map((cell) => cell.getData()));
    return { success: true };
  };
  ...
  renameNode = async (nodeInstanceId: string, newName: string) => {
    ...
    if (renameRes.success) {
      ...
      this.loadExperimentGraph(experimentId);
    }
    return renameRes;
  };
